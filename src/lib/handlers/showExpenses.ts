...
export default function handlerShowExpenses() {
	...
	if ($templateExpenseItem instanceof HTMLTemplateElement) {
		...
		expenses.map(({ id, expenseName, expenseAmount, createAt, ...rest }) => {
			...
			if ($section instanceof HTMLElement) {
				...
				$section.querySelector("#expense-item-title")!.textContent =
					expenseName;
